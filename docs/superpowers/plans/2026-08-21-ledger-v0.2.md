...
**Party convention — the refund branch MUST swap the decoded addresses.** Across this codebase `sender_address` is always the counterparty and `receiver_address` is always the merchant, for payments and refunds alike; only `tx_type` carries direction (see `simulate.py`'s refund emission, which uses the original payer as sender, and `categorize.py`'s payer axis, which groups by `sender_address`). On an *outbound* Transfer log the ERC-20 `from` is the merchant and `to` is the payer, so decoding one directly yields the fields backwards. The refund branch must swap them via `dataclasses.replace` before returning. Leave `log_to_transaction` alone — it is correct as a raw-log decoder, and this is a domain concern belonging to `fetch.py`.
